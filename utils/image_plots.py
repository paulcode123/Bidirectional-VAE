...
def display_image_in_grid(grid, label_grid=None):
    x = len(grid)
    y = len(grid[0])
    # img = mpimg.imread('traj_109_8.jpg')

    fig, axarr = plt.subplots(x, y, figsize=(int(40 * (y / 6)), int(60 * (x / 11))))
    # ims = [img for i in range(x*y)]
    ...
    for idx1, row in enumerate(grid):
        # cv2 change BGR to RGB for image row[0]
        # if row[0] is 3 channel rgb
        if len(row[0].shape) == 3:
            row[0] = cv2.cvtColor(row[0], cv2.COLOR_BGR2RGB)
            axarr[idx1][0].imshow(row[0], label="test")
            axarr[idx1][0].title.set_text("test, " + str(label_grid[idx1][0]))
            for idx2, img in enumerate(row[1:]):
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                axarr[idx1][idx2 + 1].imshow(img, label="nbhr")
                axarr[idx1][idx2 + 1].title.set_text(
                    "nbhr, " + str(label_grid[idx1][idx2 + 1])
                )
        else:
            axarr[idx1][0].imshow(row[0], label="test", cmap="gray")
            axarr[idx1][0].title.set_text("test")
            for idx2, img in enumerate(row[1:]):
                axarr[idx1][idx2 + 1].imshow(img, label="nbhr", cmap="gray")
                axarr[idx1][idx2 + 1].title.set_text(
                    "nbhr, " + str(label_grid[idx1][idx2 + 1])
                )
